...
- **Call:** `InitializeWebSocket("ws://localhost:5000/ws");` then `SendSessionInitialized();`.
- **What it does:**
...
|---------------------|-----------|----------------------|
| **session_initialized** | Once after connect (`SendSessionInitialized`) | `eventType: "session_initialized"`, `client: "EGM_Application"`, `timestamp`, `payload.availableCredits` (current credits) |
...
| **EGM → Server** sends `session_initialized` with `client: "EGM_Application"` to identify EGM | EGM sends `session_initialized` once on startup (with `payload.availableCredits`); server tags the socket as EGM and forwards session state to Roulette. |
...
- **Property names:** The server doc mentions the server may read `EventType`, `eventType`, or `event`. The EGM uses `EventType` for main events and `eventType` for session_initialized. The server must accept at least `EventType` and/or `eventType` for EGM-originated messages.
...
- **ui_ping / UI_PING:** The server doc lists `ui_ping` / `UI_PING` as EGM → server keep-alive. The EGM code does not send `ui_ping` in the snippets we have; it only sends `session_initialized` once at startup. So either keep-alive is not implemented in the EGM yet, or it is sent from somewhere else.
...
```
  ...
  │  Startup ──► SendSessionInitialized (session_initialized once), TEST ─────────────┘                │
  │                                                                   │                 │
